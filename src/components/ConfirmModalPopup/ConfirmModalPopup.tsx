        ...
        <Modal
          ...
          closeIcon={
            // props?.primaryBtn?.text ? (
              // <Text className="Infi-Sp_03_CloseIcon cls-close-modal-icon"></Text>
            // ) : (
              false
            // )
          }
            ...
            <Row className="cls-modal mb-5" justify="center">
              <Col xs={24} xl={24}>
                {props?.list?.map((list: any) => (
                  <Text className="f-reg fs-16 text-center cls-grey pb-15">{list}</Text>
                ))}
              </Col>
            </Row> : <></>
          }
          <Row className="cls-modal" justify="center">
            ...
            <Col xs={11} xl={8} className="mt-20 cls-btn" offset={1}>
              <Button
                type="primary"
                block
                className={`cls-modal-btn f-med ${(props.modalName === "declineOffer" || props.modalName === "cancel") && confirmBtn ? "disabled no-events" : ""} ${isSmallScreen ? "cls-primaryBtn-inverted" : ""}`}
                size="large"
                onClick={handleOk}
