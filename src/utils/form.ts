...
const onlyCharacter=(message:string)=>(value:string)=>{
  const charPattern= /^[a-zA-Z]+$/;

  if (!value || charPattern.test(value)) {
    // If the value is empty or it matches the OTP pattern, consider it valid
    return true;
  } else {
    // If it doesn't match the OTP pattern, provide an error message
...
export { formErrorObjectFromResponse, formToUpperCase, validateLastNameOrEmail,validateOTP, formFinishFailedFocus , onlyCharacter};
